...
fn test_satisfied(_: &Path, input: String) -> datatest_stable::Result<()> {
    ...
    assert!(verify_satisfied(&cdcl.cnf_formula));
    Ok(())
}
...
fn test_sat_with_preprocess(_: &Path, input: String) -> datatest_stable::Result<()> {
...
fn test_unsat_with_preprocess(_: &Path, input: String) -> datatest_stable::Result<()> {
    let cnf_pre = parse(input.as_str()).unwrap();
...
datatest_stable::harness! {
    { test = test_sat_with_preprocess, root = "./inputs/sat", pattern = r"^.*\.cnf$" },
    { test = test_unsat_with_preprocess, root = "./inputs/unsat", pattern = r"^.*\.cnf$" },
